...
// User-portal counterpart of admin/pages/DetectionSettings.jsx — same
// Camera fields + Unknown Person Save Cooldown, same /settings endpoint
// (never admin-gated on the old combined page), same single Reset to
// Defaults button (POST /settings/reset also resets Company Settings'
// Dashboard Refresh Interval — disclosed below, same as the Admin page).
  ...
  const handleSave = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setToast({ type: "success", message: "Detection settings saved successfully." });
      })
      .catch((err) => {
    ...
    <div>
      ...
      <Modal open={confirmReset} onClose={() => setConfirmReset(false)} title="Reset Detection Settings to Defaults">
        <p className="text-sm text-ink-300">
          This resets your Camera and detection/alert settings back to their factory defaults — this also includes
          Company Settings &gt; Dashboard Refresh Interval, since both are stored together.
        </p>
