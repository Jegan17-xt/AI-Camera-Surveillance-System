...
// User-portal counterpart of admin/pages/AISettings.jsx — same fields,
// same endpoints, same behavior. ManagingSettingsFor never renders here
// (useSettingsTarget's isCompanyAdmin is always false for a User
// session), so every request always targets the logged-in User's own
// account. Copied verbatim from the old combined pages/Settings.jsx's
// AI_DETECTION_FIELDS / RECOGNITION_NUMBER_FIELDS — neither was ever
// admin-gated there, so nothing here is new functionality for a User.
  ...
  return (
    <div>
      ...
      <Modal open={confirmReset} onClose={() => setConfirmReset(false)} title="Reset AI Settings to Defaults">
        <p className="text-sm text-ink-300">This resets your AI Detection and Recognition settings back to their factory defaults.</p>
