...
// Copied verbatim from the old single Settings page's CLEAR_ACTIONS.
const CLEAR_ACTIONS = {
  unknownImages: {
...
export default function CompanySettings() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .catch((err) => {
        ...
        setError("Unable to load company settings. Please check the server and try again.");
      })
  ...
  const handleSave = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setToast({
          type: "success",
          message: targetUserId ? `Company settings saved for ${selectedUser?.name || "this user"}.` : "Company settings saved successfully.",
        });
      })
      .catch((err) => {
        console.error("Save Company Settings API Error :", err);
        setToast({ type: "error", message: err.response?.data?.message || "Failed to save company settings." });
      })
  ...
  return (
    <div>
      <PageHeader eyebrow="Settings" title="Company Settings" description="Company profile, account preferences, and data management." />
      <ManagingSettingsFor
